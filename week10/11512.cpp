// Problem 11512 - GATTACA
// Vincent Cote - CPSC 3200
// Approach - Using given code library for suffix array, from each string build the sarray
// and compute the lcp. Run through a loop for each character of the array and check its
// lcp, find the largest one and save the substring. If the lcp is the same as max check if
// the substring is the same, if so increase how many times it has been found by 1.
    ...
    compute_lcp(str, sarray, lcp);
		...
		for (int i = 0; i < str.length(); i++) {
			if (maxSub < lcp[i]) {
				maxSub = lcp[i];
				foundNum = 2;
				subStr = str.substr(sarray[i-1], lcp[i]);
			} else if (lcp[i] !=0 && lcp[i] == maxSub && str.substr(sarray[i-1], lcp[i]) == subStr) {
				foundNum++;
			}
		}

		if (foundNum > 1) {
			cout<<subStr<<" "<<foundNum<<endl;
		} else {
