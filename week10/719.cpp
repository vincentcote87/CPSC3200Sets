...
// Approach - Using provided code for suffix array, first append the string to itself and
// run it through the sa. After this is done extract the first sa with at least as many
// characters as the original string. I then grab this string and cut it off at the same
// size as the original string and compare it to the double string starting at index 0,
// if I get a match return the result else increase result by 1.
...
int main(void)
{
  ...
  while (n--) {
		...
		cin >> str;
		string s = str;
		string doublStr = str + str;
		string x;
    build_sarray(doublStr, sarray);
		for (int i = 0; i < doublStr.length(); i++) {
			if (doublStr.length() - sarray[i] >= str.length()) {
				x = doublStr.substr(sarray[i], doublStr.length()-sarray[i]);
				break;
			}
		}
		int result = 1;
		x = x.substr(0, str.length());
		for (int i = 0; i < str.length(); i++) {
			if (x == doublStr.substr(i, str.length()))
				break;
			else
				result++;
		}
